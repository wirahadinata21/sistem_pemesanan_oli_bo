        ...
        <div class="w-full px-3 mb-6 e">
            <div class="relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="flex-auto p-4">
                    ...
                    <h5 class="font-bold py-3">Tambah Karyawan</h5>
                                ...
                                <div class="flex flex-col h-full">
                                    ...
                                    <div class="mb-4">
                                        <input wire:model.lazy="nik" type="text"
                                            ...
                                            placeholder="Nomor induk karyawan..." id="nik" required />
                                        @error('user.name') <p class="text-size-sm text-red-500">{{ $message }}</p>
                                        @enderror
                                    </div>

                                    <h6 class="font-bold leading-tight uppercase text-size-xs text-slate-500">Nama Karyawan
                                    </h6>
                                    <div class="mb-4">
                                        <input wire:model.lazy="namakaryawan" type="text"
                                            class="text-size-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow"
                                            placeholder="Nama karyawan..." id="nik" required />
                                        @error('user.name') <p class="text-size-sm text-red-500">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <select name="iddepartemen" wire:model="iddepartemen" class="text-size-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow">
                                            <option value=''>Pilih Karyawan</option>
                                            @foreach ($departemen as $iddepartemen => $deskripsi)
                                                <option value="{{ $iddepartemen }}">{{ $deskripsi }}</option>
                                            @endforeach
                                        </select>
                                    </div>
