        ...
        <div class="w-full px-3 mb-6 e">
            <div class="relative flex flex-col min-w-0 break-words bg-white shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="flex-auto p-4">
                    ...
                    <h5 class="font-bold py-3">Tambah Barang</h5>
                        ...
                        <div class="flex flex-wrap -mx-3">
                            <div class="max-w-full px-3 w-1/2 lg:flex-none">
                                <div class="flex flex-col h-full">
                                    <h6 class="font-bold leading-tight uppercase text-size-xs text-slate-500">Nama Barang
                                    </h6>

                                    <div class="mb-4">
                                        <input wire:model.lazy="namabarang" type="text"
                                    ...
                                    <h6 class="font-bold leading-tight uppercase text-size-xs text-slate-500">Jenis Barang
                                    </h6>
                                    <div class="mb-4">
                                        
                                        <select name="idjenisbarang" wire:model="idjenisbarang" class="text-size-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow">
                                            <option value=''>Pilih Jenis Barang</option>
                                            @foreach ($jenisbarang as $idjenisbarang => $deskripsi)
                                                <option value="{{ $idjenisbarang }}">{{ $deskripsi }}</option>
                                            @endforeach
                                        </select>
                                    </div>

                                    <h6 class="font-bold leading-tight uppercase text-size-xs text-slate-500">Jenis Satuan
                                    </h6>
                                    <div class="mb-4">
                                        
                                        <select name="idjenissatuan" wire:model="idjenissatuan" class="text-size-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow">
                                            <option value=''>Pilih Jenis Satuan</option>
                                            @foreach ($jenissatuan as $idjenissatuan => $deskripsi)
                                                <option value="{{ $idjenissatuan }}">{{ $deskripsi }}</option>
                                            @endforeach
                                        </select>
                                    </div>
